...
from atpl_erp.atpl_erp.report.enquiry.enquiry import get_columns
import frappe
...
def get_data(filters):
    ...
    {where_clause}
    order by seq, claim""".format(where_clause=where_clause), filters, as_dict=True, debug=True)

    if not data:
        return [], []

    df = pandas.DataFrame.from_records(data)
    df1 = pandas.pivot_table(df,
        index=["posting_date", "claim", "description", "town_worked_cf", "travel_from_cf", "travel_to_cf"],
        values=["amount"],
    ...
    columns = [
        ...
        dict(label="Travel From", fieldname="travel_from_cf", fieldtype="Data", width=100),
        dict(label="Travel To", fieldname="travel_to_cf", fieldtype="Data", width=100),
        dict(label="Claim", fieldname="claim", fieldtype="Link", options="Expense Claim", width=180),
        dict(label="Description", fieldname="description", fieldtype="Data", width=200), ]
    for col in df1.columns.to_list():
        columns += [dict(label=col, fieldname=col, fieldtype="Currency", width=100), ]
